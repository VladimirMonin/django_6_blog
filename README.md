...
Перед изменениями читай [`AGENTS.md`](AGENTS.md). Он маршрутизирует к атомарным инструкциям в [`instructions/`](instructions/). В инструкции попадают только фундаментальные правила проекта: архитектурные границы, safety gates, workflow и устойчивые контракты. Текущие планы, release notes и временные отчёты туда не добавляются.
