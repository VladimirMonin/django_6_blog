...
10. `blog.seo.social_image_context()` is the sole social-image contract. Public `og:site_name` and JSON-LD `WebSite.name` are `Exception Blog`. The contract emits exactly one `og:image`, `og:image:secure_url`, `og:image:type`, `og:image:width=1200`, `og:image:height=630`, `og:image:alt`, `twitter:card=summary_large_image`, `twitter:image` and `twitter:image:alt` on home, about, series and detail. Its fallback is the committed raster `static/images/django-6-blog-social.png` (1200×630, `image/png`); detail uses the stable same-origin `cover_media.thumbnail_og_url` route with cover-specific alt when available. Detail JSON-LD `image` must equal the selected social URL. The public media route streams a fresh private-storage read only for media owned by a published, non-deleted post; no signed query or storage host may enter social metadata, JSON-LD or persisted HTML. `request.build_absolute_uri()` makes the same-origin media/static path absolute without duplicate hosts. The base template must declare 16×16/32×32 PNG, ICO and Apple touch icon through `{% static %}` so manifest URLs remain safe. The unauthenticated root `/favicon.ico` must return 200 `image/x-icon` independently of `DEBUG`, with a deliberate public cache header; its ICO contains 16×16, 32×32 and 48×48 variants. Telegram caches its `WebPage` preview independently; after changing an already-shared page preview, refresh the canonical URL through `@WebpageBot` before user-visible re-verification.
