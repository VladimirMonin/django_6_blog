...
10. `blog.seo.social_image_context()` is the sole social-image contract. It emits exactly one `og:image`, `og:image:secure_url`, `og:image:type`, `og:image:width=1200`, `og:image:height=630`, `og:image:alt`, `twitter:card=summary_large_image`, `twitter:image` and `twitter:image:alt` on home, about, series and detail. Its fallback is the committed raster `static/images/django-6-blog-social.png` (1200×630, `image/png`); detail uses `cover_media.thumbnail_og_url` with cover-specific alt when available. Detail JSON-LD `image` must equal the selected social URL. URLs use `request.build_absolute_uri(storage_url)`: relative media/static URLs become absolute, while already absolute S3/CDN URLs remain unchanged and must never become `https://hosthttps://cdn/...`. The base template must declare 16×16/32×32 PNG, ICO and Apple touch icon through `{% static %}` so manifest URLs remain safe. The unauthenticated root `/favicon.ico` must return 200 `image/x-icon` independently of `DEBUG`, with a deliberate public cache header; its ICO contains 16×16, 32×32 and 48×48 variants.
