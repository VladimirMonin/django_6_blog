...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not docs_dir.exists():
            self.stdout.write(
                self.style.ERROR(f'❌ Директория не найдена: {docs_dir}')
            )
            return
        ...
        if options['clear']:
            ...
            if existing_count > 0:
                ...
                self.stdout.write(
                    self.style.SUCCESS(f'🗑️  Удалено {existing_count} существующих постов.')
                )
        ...
        for md_file in md_files:
            try:
                ...
                if Post.objects.filter(slug=slug).exists():
                    ...
                    self.stdout.write(
                        self.style.WARNING(f'⏭️  Пропущен: {title} (slug "{slug}" уже существует)')
                    )
                    continue
                ...
                self.stdout.write(
                    self.style.SUCCESS(f'✓ Создан: {post.title}')
                )
        ...
        if skipped_posts:
            self.stdout.write(
                self.style.WARNING(
                    f'⏭️  Пропущено постов: {len(skipped_posts)}'
                )
            )
        self.stdout.write(
