...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS(f'Найдено постов: {posts.count()}')
        )
        ...
        for post in posts:
            ...
            if post.slug != new_slug:
                self.stdout.write(
                    f'Пост #{post.id}: "{post.title}"'
                )
                self.stdout.write(
                    f'  Старый слаг: {post.slug}'
                )
                self.stdout.write(
                    self.style.SUCCESS(f'  Новый слаг:  {new_slug}')
                )
        ...
        if dry_run:
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f'✓ Обновлено слагов: {updated_count}'
                )
            )
            self.stdout.write(
