...
- Git staging/commit/push safety;
...
7. Для Git workflow не дублируй правила: канон — `instructions/DOCS.git.instructions.md`.
...
7. Перед Git-действиями прочитать `instructions/DOCS.git.instructions.md`; перед commit проверить полный staged diff, staged paths и отсутствие секретов/локальных артефактов.
...
flowchart TD
    ...
    I --> K[Read DOCS.git + inspect staged diff]
    K --> J[Commit / push only by explicit request]
```
