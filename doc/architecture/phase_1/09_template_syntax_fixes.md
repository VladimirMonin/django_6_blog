...
Django Template Engine **не умеет** арифметику внутри `{% if %}` с несколькими условиями!
...
| Подход | Код | Проблема |
|--------|-----|----------|
| ❌ Минус в if | `{% if x < y - 2 %}` | TemplateSyntaxError |
| ✅ Фильтр add | `{% if x < y\|add:"-2" %}` | Работает |
...
```mermaid
graph TD
    A[button.html] --> B[Переместить в Python]
    C[alert.html] --> B
    D[paginator.html] --> E[Использовать add filter]
    B --> F[Тесты пройдены]
    E --> F
```
