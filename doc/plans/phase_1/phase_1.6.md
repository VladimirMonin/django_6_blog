...
- [ ] Обновить `templates/blog/post_list.html`:
  - Выделить карточку поста в partial `{% #post_card %}`
  - Выделить пустое состояние в partial `{% #empty_state %}`
  - Сохранить основную структуру страницы
- [ ] Проверить корректность отображения

### 2. Рефакторинг шаблона детального просмотра

- [ ] Обновить `templates/blog/post_detail.html`:
  - Выделить заголовок поста в partial `{% #post_header %}`
  - Выделить контент поста в partial `{% #post_content %}`
  - Выделить метаданные в partial `{% #post_meta %}`
- [ ] Проверить корректность отображения

### 3. Создание базовых partials

- [ ] В `base.html` создать переиспользуемые partials:
  - `{% #navigation %}` — навигационное меню
  - `{% #footer %}` — подвал сайта (если будет)
  - Оставить основную структуру нетронутой

### 4. Документация использования

- [ ] Создать комментарии в шаблонах:
...
- [ ] Проверить все страницы работают корректно
- [ ] Убедиться что стили применяются правильно
- [ ] Проверить что partials можно вызвать отдельно
